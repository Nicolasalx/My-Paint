...
{
    sfSprite_setPosition(GET_DATA(selected_layer, layer_t)->render_sprite,
        (sfVector2f) OVERVIEW_POS);
    sfSprite_setScale(GET_DATA(selected_layer, layer_t)->render_sprite,
        (sfVector2f) OVERVIEW_SCALE);
    sfRenderWindow_drawSprite(window, GET_DATA(selected_layer, layer_t)->
        render_sprite, NULL);
    sfRectangleShape_setOutlineThickness(background, 6);
    sfRectangleShape_setPosition(background, (sfVector2f) OVERVIEW_POS);
    sfRectangleShape_setSize(background, (sfVector2f) {render_sheet_res.x,
        render_sheet_res.y});
