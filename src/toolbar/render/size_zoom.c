...
{
    if (mouse_pos.x >= zoom_shape[1].pos.x && mouse_pos.x <= zoom_shape[1].pos.x + zoom_shape[0].size.x) {
        if (mouse_pos.y >= zoom_shape[1].pos.y && mouse_pos.y <= zoom_shape[1].pos.y + zoom_shape[0].size.y) {
...
void set_zoom_size(sfRenderWindow *window, sfEvent *event)
{
    ...
    for (int i = 0; i < size_zoom_shape; ++i) {
        sfRenderWindow_drawRectangleShape(window, zoom_shape[i].rectangle, NULL);
    }
}
