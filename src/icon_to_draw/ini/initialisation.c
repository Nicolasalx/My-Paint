...
void initialise_icon_to_draw(int i)
{
    ...
    sfCircleShape_setFillColor(icon_to_draw[i].circle,
        icon_to_draw[i].color_circle);
    sfCircleShape_setPosition(icon_to_draw[i].circle,
        icon_to_draw[i].pos_circle);

    icon_to_draw[i].texture =
        sfTexture_createFromFile(icon_to_draw[i].path_img, NULL);
    icon_to_draw[i].sprite = sfSprite_create();
    sfSprite_setTexture(icon_to_draw[i].sprite,
        icon_to_draw[i].texture, sfTrue);
    sfSprite_setScale(icon_to_draw[i].sprite,
        icon_to_draw[i].size_sprite);
}
