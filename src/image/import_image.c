...
bool import_image(char *image_path)
{
    ...
    sfRenderTexture_drawSprite(GET_DATA(selected_layer,
        layer_t)->render_texture, GET_DATA(selected_layer,
            layer_t)->render_sprite, NULL);
    ...
    GET_DATA(selected_layer, layer_t)->current_version = GET_DATA(
        selected_layer, layer_t)->head_undo;
    return true;
}
