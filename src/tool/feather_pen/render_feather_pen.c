...
void render_feather_pen(void)
{
    if (sfMouse_isButtonPressed(sfMouseLeft) == true) {
        ...
        sprite_draw_pos.x -= (((float) feather_pen.texture_size.x / feather_pen.texture_size.y) * feather_pen.radius) / 2.0f;
        sprite_draw_pos.y -= feather_pen.radius / 2.0f;

        sfSprite_setPosition(feather_pen.sprite, sprite_draw_pos);
        sfSprite_setScale(feather_pen.sprite, (sfVector2f) 
            {(float) feather_pen.radius / feather_pen.texture_size.y,
            (float) feather_pen.radius / feather_pen.texture_size.y});
