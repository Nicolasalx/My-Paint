...
void compute_pos_ui_layer(sfRenderWindow *window, sfEvent *event, float *pos_y)
{
    ...
    set_pos_rect_with_anchor(background_ui_layer, (sfVector2f) {-30, 750}, TOP_RIGHT);
...
{
    set_pos_rect_with_anchor(GET_DATA(current, layer_t)->ui_layer.box_ui_layer.box, current_pos, TOP_RIGHT);
    sfVector2f rect_pos = sfRectangleShape_getPosition(GET_DATA(current, layer_t)->ui_layer.box_ui_layer.box);
    sfText_setPosition(GET_DATA(current, layer_t)->ui_layer.box_ui_layer.text,
        (sfVector2f) {rect_pos.x - 210, rect_pos.y + 10});
    sfRectangleShape_setPosition(GET_DATA(current, layer_t)->ui_layer.eye_ui_layer.box,
        (sfVector2f) {rect_pos.x - 260, rect_pos.y + 10});
    sfSprite_setPosition(GET_DATA(current, layer_t)->ui_layer.eye_ui_layer.sprite,
...
void render_layer_box_ui_layer(sfRenderWindow *window, node_t *current)
{
    if (current == selected_layer) {
        sfRectangleShape_setFillColor(GET_DATA(current, layer_t)->ui_layer.box_ui_layer.box, (sfColor) {SELECTED_LAYER_COLOR});
    } else {
        sfRectangleShape_setFillColor(GET_DATA(current, layer_t)->ui_layer.box_ui_layer.box, sfWhite);
    }
...
void render_ui_layer(sfRenderWindow *window, sfEvent *event)
{
    ...
    do {
        ...
        if (mouse_button_pressed &&
            is_mouse_over_rectangle_shape(GET_DATA(current, layer_t)->ui_layer.eye_ui_layer.box) &&
            is_mouse_over_rectangle_shape(background_ui_layer)) {
            GET_DATA(current, layer_t)->hidden = !GET_DATA(current, layer_t)->hidden;
        } else if (mouse_button_pressed &&
            is_mouse_over_rectangle_shape(GET_DATA(current, layer_t)->ui_layer.box_ui_layer.box) &&
            is_mouse_over_rectangle_shape(background_ui_layer)) {
