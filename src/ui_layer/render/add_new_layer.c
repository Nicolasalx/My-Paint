...
void add_new_layer(void)
{
    ...
    sfRenderTexture_clear(
        GET_DATA(head_layer->prev, layer_t)->render_texture, sfTransparent);
    sfRenderTexture_display(
        GET_DATA(head_layer->prev, layer_t)->render_texture);
    ...
    GET_DATA(head_layer->prev, layer_t)->current_version =
        GET_DATA(head_layer->prev, layer_t)->head_undo;
}
