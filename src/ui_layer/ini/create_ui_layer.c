...
void create_button_ui_layer(void)
{
    for (int i = 0; i < size_button_ui_layer; ++i) {
        ...
        sfRectangleShape_setSize(button_ui_layer[i].box, button_ui_layer[i].size);
        ...
        button_ui_layer[i].texture = sfTexture_createFromFile(button_ui_layer[i].path, NULL);
        sfSprite_setTexture(button_ui_layer[i].sprite, button_ui_layer[i].texture, sfTrue);
